...
from app.schemas.scheme_schema import (
    ...
    SchemeRead,
    SchemeSuccessResponse,
)

scheme_router = APIRouter(prefix="/api/schemes", tags=["Scheme"])


def ensure_admin_or_employee(current_user: dict):
    if current_user["role"] not in {"admin", "employee"}:
        raise UnauthorizedAccess(
            status_code=403, detail="Access denied: Only admin or employee allowed"
        )


@scheme_router.post(
    "/create", response_model=SchemeSuccessResponse, status_code=status.HTTP_201_CREATED
)
async def create_scheme(
    ...
    if existing:
        raise SchemeAlreadyExists(status_code=400, detail="Scheme already exists")
...
async def delete_scheme(
    ...
    return SchemeDeleteResponse(
        message="Scheme Deleted Successfully", status=status.HTTP_200_OK
    )
